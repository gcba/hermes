    ...
    public function run()
    {
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'add'          => 0,
                'delete'       => 0,
                'details'      => '{"validation":{"rules":["required","string","min:1","max:15","unique:browsers,name"],"messages":{"required":"Falta el campo :attribute.","string":"El campo :attribute debe tener texto.","max":"El campo :attribute puede tener hasta :max carácteres.","min":"El campo :attribute debe tener al menos :min carácteres."}}}',
                'order'        => 2,
            ])->save();
        }
